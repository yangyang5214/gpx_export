...
def download_garmin_data(api, activity_id, folder):
    try:
        ...
        for file_info in zip_file.infolist():
            ...
            p = os.path.join(folder, f"{activity_id}_ACTIVITY.fit")
            if os.path.exists(p):
                os.rename(p, os.path.join(folder, f"{activity_id}.fit"))
            else:
                gpx = os.path.join(folder, f"{activity_id}_ACTIVITY.gpx")
                print(f'find gpx_id {activity_id}')
                os.remove(gpx)
        os.remove(file_path)
...
def get_downloaded_ids(folder):
    return [
        ...
        if i.endswith(".fit") and not i.startswith(".")
    ]
